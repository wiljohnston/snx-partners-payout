...
class SafeBatchSubmitter {
    constructor({ network, signer, safeAddress }) {
        ...
        this.service = new SafeServiceClient(
            `https://safe-transaction${network === 'homestead' ? '' : '.' + network}.gnosis.io`
        );
    }
    ...
    async submit() {
        ...
        try {
            await service.proposeTransaction({ safeAddress, senderAddress, safeTransaction: batchTxn, safeTxHash: txHash })
